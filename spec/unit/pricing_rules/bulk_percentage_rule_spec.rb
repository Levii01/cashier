...
RSpec.describe PricingRules::BulkPercentageRule do
  let(:product) { Product.new(code: 'CF1', name: 'Coffee', price: 1123) }
  let(:rule)    { described_class.new(product_code: 'CF1', threshold: 3, percentage: 1.0 / 3) }
  let(:quantity) { 3 }
  ...
  describe '#apply' do
    ...
    context 'when basket item qualifies for bulk percentage discount' do
      context 'with 3 quantities of product' do
        let(:quantity) { 3 }

        it 'applies 33% discount on all basket items' do
          apply

          expect(basket_item.total_price).to eq(3369)
          expect(basket_item.discount).to eq(1123)
          expect(basket_item.discounted_price).to eq(2246)
        end
      end
      ...
      context 'with 5 quantities of product' do
        ...
        it 'applies 33% discount on all basket items' do
          ...
          expect(basket_item.total_price).to eq(5615)
          expect(basket_item.discount).to eq(1872)
          expect(basket_item.discounted_price).to eq(3743)
        end
      end
    end
    ...
    context 'when basket item does not qualify' do
      context 'with 2 quantities of product' do
        ...
        it 'does not apply discount if quantity < threshold' do
          ...
          expect(basket_item.total_price).to eq(2246)
